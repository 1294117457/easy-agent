...
import type { StandardSchema } from '@/stores/workflow';
...
async function handleValidate() {
  ...
  if (!validationData) {
    alert(`验证失败: ${'error' in result ? result.error : '未知错误'}`);
    return;
  }
  if (validationData.valid) {
    alert('工作流验证通过！');
  } else {
    alert(`验证失败: ${validationData.errors.join(', ')}`);
  }
}
